...
router.post("/", ensureLoggedIn, ensureAdmin, async function (req, res, next) {
   ...
   * - title ILIKE (will find case-insensitive, partial matches)
   * - minSalary (inclusive)
   * - hasEquity (finds any non zero equity)
   *
  ...
  router.get("/", async function (req, res, next) {
    try {
      if (Object.keys(req.query).length !== 0) {
        const jobs = await Job.findAllFilter(req.query);     
  ...
  router.get("/:id", async function (req, res, next) {
    try {
      try {
        let idType = parseInt(req.params.id);
      } catch (err) {
