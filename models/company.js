...
class Company {
  ...
  /** Find all companies that match filter.
   * 
   * Returns [{ handle, name, description, numEmployees, logoUrl }, ...]
  ...
  static async findAllFilter(filterData) {
     ...
     * */
    const possibleKeys = ['name', 'minEmployees', 'maxEmployees']
    ...
    const filteredCompanies = await db.query(filterSql, [...values]);
    console.log('row count is ', filteredCompanies.rowCount);
